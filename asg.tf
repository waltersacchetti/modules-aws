  ...
  block_device_mappings = length(each.value.block_device_mappings) == 0 ? [] : [
    for key, value in each.value.block_device_mappings :
    {
      ...
      ebs = value.ebs != null ? {
        ...
        kms_key_id            = value.ebs.encrypted == false || value.ebs.kms_key_id == null ? null : startswith(value.ebs.kms_key_id, "arn") ? value.ebs.kms_key_id : module.kms[value.ebs.kms_key_id].key_arn
